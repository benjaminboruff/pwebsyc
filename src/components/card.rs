...
pub fn Card<G: Html>(cx: Scope, props: CardProps) -> View<G> {
    view! {cx,
        div(class="flex flex-col items-center border rounded-lg m-2 p-2 bg-sky-50 shadow-md md:w-1/2 md:mx-auto"){
            div(class="prose"){
                p(class="text-center font-medium") { (props.repo.name)  }
                p(class="justify-left text-sm -mt-4 pb-2") { (props.repo.description) }
            }
